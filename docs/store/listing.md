...
|---|---|---|---|---|---|---|
| Product Interaction | Usage Data | PostHog | No | No | Analytics | Four events only: `deep_link_open` (path), `workout_start` (minutes), `workout_complete` (minutes, close, first), `trial_start` (plan). No screen views, lifecycle, device model or locale. Autocapture, session replay, surveys and GeoIP are off. Events queue offline. Ships only when `EXPO_PUBLIC_POSTHOG_KEY` is set in the production build. |
