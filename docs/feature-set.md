...
| Account (sign out, erase everything) | A Settings card naming how she is continuing, with sign out and an in-card confirmed erase of everything on this phone — the deletion path App Review requires once Sign in with Apple exists. | No server: erasing the phone is the deletion. The safe action is primary; nothing is a system dialog (ADR-0011, launch checklist). | Built |
...
| Voice audio | Each movement's cue read aloud during the work set by the app's one coach voice: generated once from `data/movements.json` by `scripts/generate-voice-audio.mjs` (ElevenLabs, keyed by cue text so nothing is billed twice), committed under `app/assets/voice/`, played offline through a static manifest. Asked once, on the way into the first session (spoken or silent, both real settings; the ask exists only once audio is bundled); Settings is the way back. Each set opens with one of the movement's in-set corrections, and the voice counts down the last five seconds of every hold and rest. Plays on quiet days too; the phone's silent switch wins. | Voice choice is a one-time taste decision made outside the build (build-system §9): the owner runs the generator with the chosen voice and key. Nothing is fetched at runtime — airplane mode changes nothing. | Built (pipeline + playback); assets pending the owner's voice |
| Crash reporting (Sentry) | Errors reach the owner before users report them: uncaught JS errors, native crashes, and a failed session save, behind one monitoring port; Sentry selected only when its DSN is configured. Crash and error data only: no tracing, no screenshots, no user id, `beforeSend` scrubs user and request. | Verified with the deliberate test crash (Settings → Developer tools) before the first TestFlight build (ADR-0016, launch checklist). | Built (owner sets the DSN and runs the test crash; docs/sentry-setup.md) |
| Analytics (PostHog) | The funnel that tests the retention thesis (ADR-0015, ADR-0024): `deep_link_open`, `first_use_entry`, `onboarding_complete`, `session_preview`, `workout_start`, `workout_complete`, `weekly_intention_set`, `share_eligible`, `share_start`, `paywall_view`, `scenario_entry`, `experiment_exposure`, `trial_start`, behind one typed port; PostHog selected only when its key is configured, the in-memory dev adapter otherwise. Anonymous: no identify, reset on sign-out, path-only deep links. | Payloads are closed unions the forbidden list cannot be represented in (a test scans the module); events queue offline; analytics never blocks anything (ADR-0015, fither-code). | Built (owner sets the key; docs/posthog-setup.md) |
...
| Feedback | In-app form in Settings → Account (ADR-0020): her words, optional email, app version and device go to an endpoint the owner runs (`EXPO_PUBLIC_FEEDBACK_URL`); queued on the phone while offline. Nothing about her training travels. |
